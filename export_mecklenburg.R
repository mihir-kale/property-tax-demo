# Export Mecklenburg County property tax pressure index to JSON for the static
# GitHub Pages demo at mihir-kale.github.io/property-tax-demo.
#
# This replicates the benchmark/breakdown logic from the Shiny app (app.R) but
# only for Mecklenburg County, using only base + {readr,dplyr,tidyr,stringr,jsonlite}
# so it runs on machines without gfortran/sf/tigris.
...
full_period_years    <- preferred_start_year:preferred_end_year
...
# ----------------------------
# 5. Property tax benchmark by year (mirror make_property_tax_benchmark_by_year)
# ----------------------------
...
# ----------------------------
# 6. Revenue breakdown by source (mirror make_revenue_breakdown, totals only)
# ----------------------------
...
# ----------------------------
# 7. Spending breakdown by area (mirror make_spending... breakdown, totals only)
# ----------------------------
...
writeLines(json, "/Users/mihirkale/repos/property_tax_project/property-tax-demo/data/mecklenburg.json")

cat("Wrote mecklenburg.json\n")
cat("Score:", s$score, "| Diff%:", round(s$difference_pct, 2), "| Diff$:", format(s$difference_dollars, big.mark=","), "\n")
